...
> **v0.5.0 · Liquid Orb 与状态主动作。** 代码实时绘制液态球，风险状态平滑衔接，容量保持独立外环。悬停预览、单击当前主动作、右键查看详情。桌面支持屏幕和窗口磁吸。真实容量、自动压缩、交接并续作尚未接入；当前动作准确提供检查准备、依据查看与简报整理。

![Context Orb v0.5 应用预览](docs/assets/preview-v05.png)

[体验 v0.5 交互预览](http://127.0.0.1:1427)（先按下方命令启动）。上图为虚构数据的实际应用截图。[代码渲染的液态动画录像](docs/assets/liquid-orb-demo.webm) · [新设计与复审记录](docs/ORB-REDESIGN-V1.md)。
...
窗口磁吸设置为「仅限 Codex 窗口｜关闭｜所有窗口」，默认 Codex。拖动后松手必靠边：在允许的窗口内或球体触及可见边框时，完整停靠窗口内侧并随该窗口移动；其余位置或窗口可见区域放不下完整球体时停靠屏幕边缘。关闭窗口吸附时仍停靠屏幕。macOS 使用系统 bundle ID 识别 Codex；Windows Codex 身份尚未验证，该档明确不可用。没有 CLI 或手动窗口绑定。拖动松手后收起详情，点击进入已展示的主动作；右键或 Shift F10 打开总览。[交互与平台边界](docs/UIUX.md)。
