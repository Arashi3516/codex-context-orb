...
test('the task brief is editable and copies exactly the reviewed text', async ({ page, context }) => {
  await context.grantPermissions(['clipboard-read', 'clipboard-write'])
  await page.getByRole('button', { name: '整理交接简报' }).click()
...
test('drag does not trigger the primary action; right click and keyboard preserve their distinct actions', async ({ page }) => {
...
test('hover previews after 350 ms without acting, and the preview stays open while reading it', async ({ page }) => {
  const now = Date.now()
  await page.clock.install({ time: now })
  await page.clock.pauseAt(now + 1000)
  await page.goto('/?surface=orb')
  const orb = page.locator('.orb-button')
  const peek = page.getByTestId('orb-peek')
  await orb.hover()
  await page.clock.runFor(349)
  await expect(peek).toHaveCount(0)
  await page.clock.runFor(1)
  await expect(peek).toBeVisible()
  await expect(peek).toContainText('查看偏差依据')
  await expect(page.getByTestId('orb-panel')).toHaveCount(0)
  await peek.hover()
...
test('keyboard focus previews, Escape dismisses it, and Shift F10 opens details without running the primary action', async ({ page }) => {
  const now = Date.now()
  await page.clock.install({ time: now })
  await page.clock.pauseAt(now + 1000)
  await page.goto('/?surface=orb')
...
test('motion can be stopped and persisted, and reduced motion stops the ball', async ({ page }) => {
  const visual = page.locator('.orb-button .orb-visual')
  ...
  await expect(visual).toHaveAttribute('data-running', 'false')
  await page.getByRole('button', { name: '提醒设置', exact: true }).click()
  await expect(page.getByRole('switch', { name: '球内流动效果' })).not.toBeChecked()
  await page.getByRole('switch', { name: '球内流动效果' }).click()
